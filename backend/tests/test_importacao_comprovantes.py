...
def test_listagem_ordena_por_data_pagamento(comprovante_client, db_fake):
    """Com ordenar_por=data_pagamento, o mais recente vem primeiro."""
    db_fake._dados["comprovantes"] = [
        {"id": 1, "tipo_documento": "Pix", "nome": None, "descricao": "Antigo",
         "data_pagamento": "2026-01-10", "data_registro": "2026-08-01T10:00:00Z"},
        {"id": 2, "tipo_documento": "Pix", "nome": None, "descricao": "Recente",
         "data_pagamento": "2026-08-14", "data_registro": "2026-08-01T11:00:00Z"},
    ]
    resp = comprovante_client.get("/api/comprovantes/?ordenar_por=data_pagamento")
    assert resp.status_code == 200, resp.text
    data = resp.json()

...
def listar_fluxos(db=Depends(get_supabase)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Erro ao buscar fluxo de caixa") from None
...
def criar_fluxo(fluxo: FluxoCaixaCreate, db=Depends(get_supabase)):
    ...
    try:
        ...
        saidas = (
            fluxo.despesa_contabilidade
            + fluxo.despesa_internet
            + fluxo.despesa_lavagem
            + fluxo.despesa_manutencao
            + fluxo.despesa_imposto
            + fluxo.despesa_taxa
            + fluxo.despesa_diversas
        )
        ...
        raise HTTPException(status_code=500, detail="Erro ao salvar fluxo de caixa") from None
...
def atualizar_fluxo(fluxo_id: int, fluxo: FluxoCaixaCreate, db=Depends(get_supabase)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Erro ao atualizar fluxo de caixa") from None
...
def excluir_fluxo(fluxo_id: int, db=Depends(get_supabase)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Erro ao excluir fluxo de caixa") from None


@router.get("/{fluxo_id}/relatorio")
def obter_relatorio_pdf(
    fluxo_id: int,
    socio: str | None = Query(
        None,
        description=(
            "Filtro para gerar relatório apenas de um sócio específico "
            "(ex: Demarco, Marlene, João B., Nei Rigo, Gilmar T.)"
        ),
    ),
    ...
    try:
        ...
        return FileResponse(caminho_pdf, media_type="application/pdf", filename=nome_download)
    except HTTPException:
        raise
    except Exception:
        logger.exception("Erro ao gerar relatório do fluxo")
        raise HTTPException(status_code=500, detail="Erro ao gerar relatório do fluxo") from None

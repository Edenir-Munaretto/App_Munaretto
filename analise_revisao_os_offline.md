...
| 2 — coerência offline/online | A5, A6, A7, A8 + filtros offline, Evidências offline, botão Impedir, guarda de clique | ✅ Implementado (set/2026): painel único por viewport (A6); corrida de listagem com geração/descarte + catálogos só no reset (A7); retries do detalhe com cleanup e `setErro('')` (A8); espelhamento online→pacote (os_lista na listagem, status, material; respostas já cobriam); filtros/busca/paginação offline no quadro ativo; botão "Impedir" no painel (decisão de negócio nº 3 atendida — mobile/campo); guardas de clique duplo (lock em `mudarStatus` + `disabled` no AcoesStatus); timer do toast com limpeza. ❌ **Fora do escopo por decisão** (sem prazo): aba **Evidências offline** e visão **Encerradas offline**; gate `exige_foto` no frontend só bloqueia online (offline o sync valida a verdade). |
| 3 — robustez backend | A2-fim… (sanitização de busca, X-Total-Count, TOCTOU código, `criar_os` transacional, uploads streaming, timeouts `apiFetch`, `_gravar_membros`, importação em lote, índice único HH aberto) | ✅ Implementado (set/2026): busca sanitizada contra a gramática do PostgREST (`_termo_busca_seguro`, os.py/apoio_os.py); `X-Total-Count` com `count="exact"` (correto acima de ~1000); código O.S com retry em violação de unicidade no INSERT (TOCTOU) e `criar_os` com rollback dos estágios (snapshot/histórico); uploads lidos em chunks de 1 MB com recusa no limite (sem carregar arquivo inteiro); mensagens 500 sem vazar exceção (4 locais); timeout padrão de 30s no `apiFetch`; `_gravar_membros` com validação de existência prévia e diff (add antes do remove) — cria/edita equipe sem estado órfão; importação em lote com catálogo em memória + inserts em lotes de 100 (era 4-6 consultas/linha); índice único parcial `fim IS NULL` para cronômetro aberto no `schema.sql`; drag&drop com os MESMOS gates de checklist dos botões (A10). Testes: retry de código, rollback, limite de upload, busca com `()`, equipes (diff/validação) — 256 testes passando. Deploy: re-rodar `schema.sql`; ANTES, fechar blocos de H.H. duplicados abertos se existirem (nota no schema). |
| 4 — higiene/baixos | (ver seção 🟢 e itens médios restantes) | ✅ Implementado (set/2026): PDFs com arquivo temporário único (mkstemp) e remoção via `BackgroundTask` (sem colisão/lixo no temp); timestamps nos PDFs no fuso `America/Sao_Paulo` (`date_helpers`); `editar_os` com `exclude_unset` (PUT parcial não zera campos); `snapshot_checklist` filtra o `tipo` do modelo e é idempotente (insere só faltantes; corrida tratada); `listar_equipes` sem N+1; `enviar_foto` com basename+truncatura (e `_remover_objeto` que faltava definido); tipos/contratos centralizados (`utils/tipos_os.py` + constantes no frontend); resumo offline com grupos dinâmicos e nomes preservados por número; `DB_VERSION=3` com migração por bloco e `onblocked/onversionchange`; polyfill `AbortSignal.timeout`; SW `munaretto-v2` com pré-cache tolerante; revoke de blob URLs de PDF; remoção de no-ops; id com `Number()`. **Reabertura de O.S concluída/cancelada implementada** (gestor + justificativa ≥ 10, limpa `data_fim`, histórico registra). 263 testes passando; ruff sem novos apontamentos; lint/build ok. Deploy: apenas backend + frontend (sem mudança de schema). |
...
## Decisões de negócio — status

1. **Custo real de M.O. zerado** no relatório — ✅ decidido: manter zerado até definir valor por equipe (documentado no código como intencional).
2. **Reabertura de O.S concluída/cancelada** pelo gestor — ✅ implementada (Lote 4): transição `-> aberta` com justificativa ≥ 10 caracteres, limpa `data_fim` e registra no histórico; botão no painel do gestor.
3. Botão **"Impedir"** no painel/mobile — ✅ implementado (Lote 2).
4. Manter O.S **concluída visível** no Modo Campo — ✅ decidido: **não** — O.S `concluida` permanece fora da faixa do campo (colunas do Modo Campo seguem `aberta/em_andamento/impedida`).
